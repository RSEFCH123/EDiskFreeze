...
NTSTATUS MyCompleteRoutine_WriteDisk(
	...
	IN ULONG EaLength)
{
	...
	{
		//DbgPrintEx(0, 0, "IoCreateFile Error[0x%X]", status);
		return status;
	}
	...
	{
		ZwClose(hRootFile);
		//DbgPrintEx(0, 0, "ObReferenceObjectByHandle Error[0x%X]\n", status);
		return status;
	}
	...
	if (NULL == pIrp)
	{
		...
		//DbgPrintEx(0, 0, "IoAllocateIrp Error!\n");
	...
	if (!NT_SUCCESS(status))
	{
		//DbgPrintEx(0, 0, "ObCreateObject Error[0x%X]\n", status);
		return status;
	}
	...
	if (!NT_SUCCESS(status))
	{
		IoFreeIrp(pIrp);
		ObDereferenceObject(pFileObject);
		//DbgPrintEx(0, 0, "SeCreateAccessState Error[0x%X]\n", status);
		return status;
	}
...
NTSTATUS IrpQueryDirectoryFileEx(
	...
	IN ULONG Length,
	IN FILE_INFORMATION_CLASS FileInformationClass,
	IN BOOLEAN RestartScan,   // 新增参数，指示是否重新开始扫描
	IN PUNICODE_STRING FileName OPTIONAL)
{
	NTSTATUS status = STATUS_SUCCESS;
	PIRP pIrp = NULL;
...
NTSTATUS _IrpSetInformationFile(
	IN PFILE_OBJECT pFileObject,
	OUT PIO_STATUS_BLOCK IoStatusBlock,
...
PFILE_OBJECT Irp_OpenFile_Unicode_string(PUNICODE_STRING ustrFileName)
{
	PFILE_OBJECT pFileObject = NULL;
	IO_STATUS_BLOCK iosb = { 0 };
	NTSTATUS status = STATUS_SUCCESS;
	status = IrpCreateFile(&pFileObject, DELETE | FILE_READ_ATTRIBUTES | SYNCHRONIZE | FILE_WRITE_ATTRIBUTES,
	...
	IN PUNICODE_STRING DevicePath)
{
	...
	// 使用IoCreateFile打开磁盘设备
	status = IoCreateFile(&hDevice, SYNCHRONIZE,
		&objAttr, &ioStatus, NULL, FILE_ATTRIBUTE_NORMAL,
		FILE_SHARE_READ | FILE_SHARE_WRITE, FILE_OPEN,
		FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0,
		CreateFileTypeNone, NULL, IO_NO_PARAMETER_CHECKING | IO_IGNORE_SHARE_ACCESS_CHECK);
	...
	// 获取文件对象
	status = ObReferenceObjectByHandle(hDevice, NULL,
		*IoFileObjectType, KernelMode, (PVOID*)ppFileObject, NULL);
	...
	IN PVOID Buffer)
{
	...
	// 基本校验
	if (!pFileObject || !Buffer || Length == 0) {
		return STATUS_INVALID_PARAMETER;
	}

	// 扇区对齐，别幻想 storport 帮你兜
	if ((ByteOffset.QuadPart & 0x1FF) || (Length & 0x1FF)) {
		return STATUS_INVALID_PARAMETER;
	}
	...
	pMdl = IoAllocateMdl(Buffer, Length, FALSE, FALSE, NULL);
	if (!pMdl) {
		IoFreeIrp(pIrp);
		return STATUS_INSUFFICIENT_RESOURCES;
	}
...
NTSTATUS WriteFile_IRP(
	_In_ PUNICODE_STRING DosFilePath,
	_In_ PVOID FileData,
	_In_ ULONG FileDataLength
)
{
	NTSTATUS status = STATUS_SUCCESS;
	PFILE_OBJECT pFileObject = NULL;
	...
	if (!NT_SUCCESS(status)) {
		DbgPrint("Open File Failed(WRITE ACCESS): 0x%X\n", status);
		status = IrpCreateFile(
			&pFileObject,
			0,
			DosFilePath,
			...
			0);
		if (!NT_SUCCESS(status)) {
			DbgPrint("Open File Failed(0 ACCESS): 0x%X\n", status);
			return status;
		}
	}
	...
	if (!NT_SUCCESS(status)) {
		DbgPrint("Write File Failed: 0x%X\n", status);
	}
	...
	OUT PVOID Buffer)
{
	...
	pIoStack->DeviceObject = pDevObj;
	pIoStack->FileObject = pFileObject;
	pIoStack->Parameters.Read.Length = Length;
	pIoStack->Parameters.Read.ByteOffset = ByteOffset;

	// 设置完成例程
	IoSetCompletionRoutine(pIrp, MyCompleteRoutine2, NULL, TRUE, TRUE, TRUE);
